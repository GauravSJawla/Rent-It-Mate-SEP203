...
router.post('/', [auth, [
    check('address1', 'Atleast one address line is required').not().isEmpty(),
    ...
    const {
        address1,address2,city,state,country,zipcode,homePhone,mobilePhone,alternateEmail
    } = req.body;
    ...
    if(homePhone) { profileFields.homePhone = homePhone;}
    if(mobilePhone) { profileFields.mobilePhone = mobilePhone;}
    if(alternateEmail) {profileFields.alternateEmail = alternateEmail;}
...
router.delete('/', auth, async(req,res) => {
    try{
        ...
        if(userProfile){
            await Profile.delete(userProfile);
        }
